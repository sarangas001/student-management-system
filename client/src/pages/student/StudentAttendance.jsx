...
const StudentAttendance = () => {
  ...
  const ATTENDANCE_EXCELLENT = 90;

  const getBadgeClass = (percentage) => {
    if (percentage >= ATTENDANCE_EXCELLENT) return "attendance-badge green";
    if (percentage >= ATTENDANCE_WARNING) return "attendance-badge light-green";
    return "attendance-badge yellow";
  };

  const atRiskCourses = attendanceData.filter((item) => item.percentage < ATTENDANCE_WARNING);

  const filteredData = filterCourse === "all"
    ? attendanceData
    : attendanceData.filter((item) => item.courseId === filterCourse);
    ...
    <div className="attendance-container">
      <div className="attendance-card">
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", flexWrap: "wrap", gap: "15px" }}>
          <h3 className="attendance-title" style={{ margin: 0 }}>My Attendance Record</h3>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <span style={{ fontSize: "14px", color: "var(--text2)", fontWeight: "500" }}>Filter Course:</span>
            <select
              value={filterCourse}
                        ...
                        <tr>
                          <td colSpan="7" style={{ backgroundColor: "var(--surface2)", padding: "20px", borderBottom: "1px solid var(--border)" }}>
                            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                              <h4 style={{ margin: 0, fontSize: "14px", fontWeight: "600", color: "var(--text)", display: "flex", alignItems: "center", gap: "6px" }}>
